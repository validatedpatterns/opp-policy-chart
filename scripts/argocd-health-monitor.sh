...
# Function to remediate a wedged cluster (force sync one or more resources in the Application, same as cron)
remediate_wedged_cluster() {
  ...
  # Prefer FORCE_SYNC_RESOURCES (JSON array); fallback to single resource
  local patch_payload
