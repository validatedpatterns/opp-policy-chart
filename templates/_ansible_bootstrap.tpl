{{/*
  Stage ConfigMap flat keys into /tmp/opp-policy-ansible (no ansible install).
  Use for long-running shell prereqs so output streams to pod logs without Ansible buffering.
*/}}
{{- define "opp.ansibleStageOnly" -}}
set -euo pipefail
export HOME=/tmp
STAGE=/tmp/opp-policy-ansible
rm -rf "$STAGE"
mkdir -p "$STAGE"
shopt -s nullglob
...
*/}}
{{- define "opp.ansibleBootstrap" -}}
{{ include "opp.ansibleStageOnly" . }}
